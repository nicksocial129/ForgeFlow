...
class ChatOllama_ChatModels {
    constructor() {
        ...
        this.baseClasses = [this.type, ...(0, utils_1.getBaseClasses)(ollama_1.ChatOllama)];
        this.inputs = [
            {
                label: 'Cache',
                name: 'cache',
    ...
    async init(nodeData) {
        ...
        const numGqa = nodeData.inputs?.numGqa;
        ...
        if (mirostat)
            obj.mirostat = parseFloat(mirostat);
        if (mirostatEta)
            obj.mirostatEta = parseFloat(mirostatEta);
        if (mirostatTau)
            obj.mirostatTau = parseFloat(mirostatTau);
        if (numCtx)
            obj.numCtx = parseFloat(numCtx);
        if (numGqa)
            obj.numGqa = parseFloat(numGqa);
        if (numGpu)
            obj.numGpu = parseFloat(numGpu);
        if (numThread)
            obj.numThread = parseFloat(numThread);
        if (repeatLastN)
            obj.repeatLastN = parseFloat(repeatLastN);
        if (repeatPenalty)
            obj.repeatPenalty = parseFloat(repeatPenalty);
        if (tfsZ)
            obj.tfsZ = parseFloat(tfsZ);
        if (stop) {
        ...
        if (cache)
            obj.cache = cache;
        const model = new ollama_1.ChatOllama(obj);
        return model;
    }
}
module.exports = { nodeClass: ChatOllama_ChatModels };
//# sourceMappingURL=ChatOllama.js.map